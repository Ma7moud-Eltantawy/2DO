...
import 'package:todoapp/local_style/Local_styles.dart';
...
showTasks({
  required context,required List<TaskModel> tasks,required height,required width
}) {
  return Stack(
    children: [
                      ...
                      SlidableAction(
                        ...
                        onPressed: (context)async  {
                          TaskModel task = TaskModel(
                            ...
                            repeat: tasks[index].repeat, id: tasks[index].id,
                          );
                          prov.updateTask(task);

                        },
                        foregroundColor: tasks[index].isFavorites==1?Colors.red:Colors.grey,
                        icon: IconBroken.Heart,
                        label:  tasks[index].isFavorites==1?'delete from favourites':"add to favourites",
                      ...
                      children: [
                        IconButton(
                          onPressed: () {
                            TaskModel task = TaskModel(
                              title:tasks[index].title ,
                              isCompleted: tasks[index].isCompleted==0?1:0,
                          ...
                          icon: tasks[index].isCompleted == 1
                              ? Icon(
                            Icons.check_box,
                            color: Choose_color[tasks[index].color!.toInt()],
                            size: 30,
                          )
                        ...
                        IconButton(
                            onPressed: (){
                              My_bottom_sheet(context,height,width,tasks[index]);
                            },
      ...
      Positioned(
        ...
        child: Padding(
          ...
          child: Mbutton(width:MediaQuery.of(context).size.width , height: MediaQuery.of(context).size.height, colors: [
          local_red,
            local_blue,
          ], txt: "Add Task", wid: Icon(IconBroken.Plus,color: Colors.white,), func: () async {
            ...
            Navigator.of(context).pushNamed(AddTask_Screen.scid);
